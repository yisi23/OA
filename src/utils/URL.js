...
URL.removePara = function (url, paraName) {
  ...
  var base = arr2[0]
  var para = arr2.length > 1 ? arr2[1] : ''
  var hash = arr1.length > 1 ? '#' + arr1[1] : ''
  var paraReg = typeof paraName === 'string' ? [paraName] : paraName.join ? paraName : []
...
URL.getInfo = function (url) {
  var fixUrl = URL.getFullPath(url || document.URL)
  var info = $.map(/^((\w+:)\/\/)?(?:(\w+):?(\w+)?@)?([^/?:]+):?(\d+)?(\/?[^?#]+)?(\??[^#]+)?(#?.*$)/.exec(fixUrl), function (val) {
    return val || ''
  })
  return {
    href: info[0],
